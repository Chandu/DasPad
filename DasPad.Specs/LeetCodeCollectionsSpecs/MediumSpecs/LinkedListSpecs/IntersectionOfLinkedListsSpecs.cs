...
{
  public class IntersectionOfLinkedListsSpecs
  {
    /*
    ...
    public ListNode GetIntersectionNodeDoesntWorkMyVersion(ListNode first, ListNode second)
    {
      ListNode intersectionPoint = new ListNode(-1);
      var curNode = intersectionPoint;
      var top = Reverse(first);
      var topHead = top;
      var bottom = Reverse(second);
      var bottomHead = bottom;
      ...
      Reverse(topHead);
      Reverse(bottomHead);
      return Reverse(intersectionPoint.next);
    }
    ...
    //[Theory]
    //[InlineData("4,1,8,4,5", "5,0,1,8,4,5", "8, 4, 5")]
    //public void CanGetIntersectionNode(string top, string bottom, string expected)
    //{
    //  Assert.Equal(expected.AsListNode(), GetIntersectionNode(top.AsListNode(), bottom.AsListNode()));
    //}
  }
}
